...
export default function FacturaForm({ id = null }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const resClientes = await axios.get(`${API_URL}/clientes/clientes_por_pagina/500/num_pagina/1`);
                setClientes(resClientes.data.data || []);

                // 2. Fetch available Juegos
                const resJuegos = await axios.get(`${API_URL}/juegos/juegos_por_pagina/500/num_pagina/1`);
                setJuegos(resJuegos.data.data || []);

                // 3. Fetch available Cuentas de Juego (Para el snapshot)
                const resCuentas = await axios.get(`${API_URL}/cuentas/cuentas_por_pagina/500/num_pagina/1`);
                setCuentas(resCuentas.data.data || []);

                // 4. If editing, fetch facture details
                if (isEditing) {
                    const resFactura = await axios.get(`${API_URL}/facturas/form_factura/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const API_URL = import.meta.env.VITE_NODE_API_URL || 'http://localhost:3000';
            if (isEditing) {
                await axios.patch(`${API_URL}/facturas/form_factura/${id}`, payload);
            } else {
                await axios.post(`${API_URL}/facturas/form_factura`, payload);
            }
